...
"""
	Moving region moving region intersects
...
def quad_roots3(a, b, c, d, o, f, g, h, k, l, m, n, p, q, r, s, it):
    global eps
    s_exec_time = time.time()

    a0 = (a*g*q**2*s - a*h*p*q*s - a*m*q**2*s + a*n*q**2*r - b*g*p*q*s + b*h*p**2*s + b*m*p*q*s - b*n*p*q*r + c*f*p*q*s + c*k*q**2*s - c*l*q**2*r - c*o*q**2*s - d*f*p**2*s - d*k*p*q*s + d*l*p*q*r + d*o*p*q*s - f*m*p*q*s + f*n*p*q*r - g*k*q**2*s + g*l*q**2*r + h*k*p*q*s - h*l*p*q*r + m*o*q**2*s - n*o*q**2*r ) / (q**2*s)
    a1 = (a*h*q*s - a*n*q**2 + b*g*q*s - 2*b*h*p*s - b*m*q*s + b*n*p*q + b*n*q*r - c*f*q*s + c*l*q**2 + 2*d*f*p*s + d*k*q*s - d*l*p*q - d*l*q*r - d*o*q*s + f*m*q*s - f*n*p*q - f*n*q*r - g*l*q**2 - h*k*q*s + h*l*p*q + h*l*q*r + n*o*q**2 ) / (q**2*s)
    a2 = (b*h*s - b*n*q - d*f*s + d*l*q + f*n*q - h*l*q ) / (q**2*s)
...
def quad_roots2(a, b, c, d, o, f, g, h, k, l, m, n, p, q, r, s, it):
    #a0 = (a*g - a*m + c*k - c*o - g*k + m*o)
    #a1 = (a*h - a*n + b*g - b*m - c*f + c*l + d*k - d*o + f*m - g*l - h*k + n*o)
    #a2 = (b*h - b*n - d*f + d*l + f*n - h*l)
...
def quad_roots4(a, b, c, d, o, f, g, h, k, l, m, n, p, q, r, s, it):
    global eps
    s_exec_time = time.time()
    ...
    a0 = (a*g*qq*s - a*h*p*q*s - a*m*qq*s + a*n*qq*r - b*g*p*q*s + b*h*pp*s + b*m*p*q*s - b*n*p*q*r + c*f*p*q*s + c*k*qq*s - c*l*qq*r - c*o*qq*s - d*f*pp*s - d*k*p*q*s + d*l*p*q*r + d*o*p*q*s - f*m*p*q*s + f*n*p*q*r - g*k*qq*s + g*l*qq*r + h*k*p*q*s - h*l*p*q*r + m*o*qq*s - n*o*qq*r ) / den
    a1 = (a*h*q*s - a*n*qq + b*g*q*s - 2*b*h*p*s - b*m*q*s + b*n*p*q + b*n*q*r - c*f*q*s + c*l*qq + 2*d*f*p*s + d*k*q*s - d*l*p*q - d*l*q*r - d*o*q*s + f*m*q*s - f*n*p*q - f*n*q*r - g*l*qq - h*k*q*s + h*l*p*q + h*l*q*r + n*o*qq ) / den
                ...
                if it0 != None:
                    ...
                    if USE_FILTER_STEP:
                        ...
                        if flag:
                            ...
                            #Intersects: [1019.24, 1978.95]
                            #Exec. Time: 25.87sec, NE: 3116
                            #Intersects: [1019.24, 1978.95]
                            #Exec. Time: 180.89sec, NE: 2472
					
                            #Intersects: [1019.24, 1978.95]
                            #Exec. Time: 23.21sec, NE: 2472
                            ...
                            (((o + f * t) - (a + b * t)) * ((m + n * t) - (c + d * t))) - (((g + h * t) - (c + d * t)) * ((k + l * t) - (a + b * t)))

                            (((o + f * ((t - p) / q)) - (a + b * ((t - p) / q))) * ((m + n * ((t - r) / s)) - (c + d * ((t - p) / q)))) - (((g + h * ((t - p) / q)) - (c + d * ((t - p) / q))) * ((k + l * ((t - r) / s)) - (a + b * ((t - p) / q))))

                            bht2-bnt2-dft2+dlt2+fnt2-hlt2+aht-ant+bgt-bmt-cft+clt+dkt-dot+fmt-glt-hkt+not+ag-am+ck-co-gk+mo

					        agq2s-ahpqs+ahqst-amq2s+anq2r-anq2t-bgpqs+bgqst+bhp2s-2bhpst+bhst2+bmpqs-bmqst-bnpqr+bnpqt+bnqrt-bnqt2+cfpqs-cfqst+ckq2s-clq2r+clq2t-coq2s-dfp2s+2dfpst-dfst2-dkpqs+dkqst+dlpqr-dlpqt-dlqrt+dlqt2+dopqs-doqst-fmpqs+fmqst+fnpqr-fnpqt-fnqrt+fnqt2-gkq2s+glq2r-glq2t+hkpqs-hkqst-hlpqr+hlpqt+hlqrt-hlqt2+moq2s-noq2r+noq2t

                            (a*g - a*m + c*k - c*o - g*k + m*o)
                            (a*h - a*n + b*g - b*m - c*f + c*l + d*k - d*o + f*m - g*l - h*k + n*o)
                            (b*h - b*n - d*f + d*l + f*n - h*l)
                            """
                            ...
                            exec_time, r01 = quad_roots4(a, b, c, d, o, f, g, h, k, l, m, n, p, q, r1, s1, it)
                            osolver_exec_time += exec_time

                            #f = (((Bx - Ax) * (Dy - Ay)) - ((By - Ay) * (Dx - Ax)))
                            #exec_time, r02 = get_solutions(f, t, it)
                            exec_time, r02 = quad_roots4(a, b, c, d, o, f, g, h, t, u, v, z, p, q, r1, s1, it)					
                            osolver_exec_time += exec_time

                            #f = (((Dx - Cx) * (Ay - Cy)) - ((Dy - Cy) * (Ax - Cx)))
                            #exec_time, r03 = get_solutions(f, t, it)
                            exec_time, r03 = quad_roots4(k, l, m, n, t, u, v, z, a, b, c, d, p, q, r1, s1, it)					
                            osolver_exec_time += exec_time

                            #f = (((Dx - Cx) * (By - Cy)) - ((Dy - Cy) * (Bx - Cx)))
                            #exec_time, r04 = get_solutions(f, t, it)
                            exec_time, r04 = quad_roots4(k, l, m, n, t, u, v, z, o, f, g, h, p, q, r1, s1, it)					
                            osolver_exec_time += exec_time
                            ...
                            #if r01 == [] and r02 == [] and r03 == [] and r04 == []:
                                col = 1
                                #print(f)
...
def get_solutions_quartic_dir(a, b, c, g, h, l, r, q, f, m, n, o, w, v, y, x, k, u, s, d, z, p, it):
    ...
    #a_exec_time = time.time()
    a0 = (2*adk*p*z + 2*adk*zz + ad*m*pp - ad*pp*x - 2*ad*p*x*z - ad*u*zz - ad*x*zz + 4*adddk*p*s*z + 4*adddk*s*zz + 4*addd*m*pp*s - 2*addd*n*pp*s - 2*addd*pp*s*x - 4*addd*p*s*x*z - 2*addd*s*u*zz - 2*addd*s*x*zz + 2*addk*p*ss*z + 2*addk*ss*zz + 6*add*m*pp*ss - 6*add*n*pp*ss + add*o*pp*ss - add*pp*ss*x - 2*add*p*ss*x*z - add*ss*u*zz - add*ss*x*zz + 4*a*dmpp*sss - 6*a*d*n*pp*sss + 2*a*d*o*pp*sss + a*m*pp*ssss - 2*a*n*pp*ssss + a*o*pp*ssss - 4*b*ddd*k*p*s*z - 4*b*ddd*k*s*zz - 2*b*ddd*m*pp*s + 2*b*ddd*pp
    a2 = (2*ad*k - ad*u - ad*x + 4*adddk*p + 4*adddk*s + 8*adddk*z - 4*addd*p*x - 2*addd*s*u - 2*addd*s*x - 4*addd*u*z - 4*addd*x*z + 4*addk*p*s + 2*addk*p*z + 2*addk*ss + 8*addk*s*z + 2*addk*zz + 6*add*m*pp - 6*add*n*pp + add*o*pp - add*pp*x - 4*add*p*s*x - 2*add*p*x*z - add*ss*u - add*ss*x - 4*add*s*u*z - 4*add*s*x*z - add*u*zz - add*x*zz + 12*a*dmpp*s - 18*a*d*n*pp*s + 6*a*d*o*pp*s + 6*a*m*pp*ss - 12*a*n*pp*ss + 6*a*o*pp*ss - 4*b*ddd*k*p - 4*b*ddd*k*s - 8*b*ddd*k*z + 4*b*ddd*p*x + 2*b*ddd*s*u + 2*b*ddd*s*x + 4*b*ddd*u*z + 4*b*ddd*x*z - 8*bddk*p*s - 4*bddk*p*z - 4*bddk*ss - 16*bddk*s*z - 4*bddk*zz - 6*bdd*m*pp + 4*bdd*n*pp + 2*bdd*pp*x + 8*bdd*p*s*x + 4*bdd*p*x*z + 2*bdd*ss*u + 2*bdd*ss*x + 8*bdd*s*u*z + 8*bdd*s*x*z + 2*bdd*u*zz + 2*bdd*x*zz - 18*b*dmpp*s + 24*b*d*n*pp*s - 6*b*d*o*pp*s - 12*b*m*pp*ss + 24*b*n*pp*ss - 12*b*o*pp*ss + 4*cddk*p*s + 2*cddk*p*z + 2*cddk*ss + 8*cddk*s*z + 2*cddk*zz + cdd*m*pp - cdd*pp*x - 4*cdd*p*s*x - 2*cdd*p*x*z - cdd*ss*u - cdd*ss*x - 4*cdd*s*u*z - 4*cdd*s*x*z - cdd*u*zz - cdd*x*zz + 6*c*dmpp*s - 6*c*d*n*pp*s + 6*c*m*pp*ss - 12*c*n*pp*ss + 6*c*o*pp*ss - 2*dddd*g*v + dddd*g*w + dddd*g*y - 2*dddd*k*r + 2*dddd*m*v - dddd*m*w - dddd*m*y + dddd*r*u + dddd*r*x - 4*ddd*g*p*v + 4*ddd*g*p*w - 4*ddd*g*s*v + 2*ddd*g*s*w + 2*ddd*g*s*y - 8*ddd*g*v*z + 4*ddd*g*w*z + 4*ddd*g*y*z + 4*ddd*h*p*v - 4*ddd*h*p*w + 4*ddd*h*s*v - 2*ddd*h*s*w - 2*ddd*h*s*y + 8*ddd*h*v*z - 4*ddd*h*w*z - 4*ddd*h*y*z + 4*ddd*k*p*q - 4*ddd*k*p*r + 4*ddd*k*q*s + 8*ddd*k*q*z - 4*ddd*k*r*s - 8*ddd*k*r*z + 4*ddd*m*p*v - 4*ddd*m*p*w + 4*ddd*m*s*v - 2*ddd*m*s*w - 2*ddd*m*s*y + 8*ddd*m*v*z - 4*ddd*m*w*z - 4*ddd*m*y*z - 4*dddn*p*v + 4*dddn*p*w - 4*dddn*s*v + 2*ddd
dd*l*s*y + 4*dd*l*v*z - 2*dd*l*w*z - 2*dd*l*y*z - 2*dd*m*p*v + 2*dd*m*p*w - 4*dd*m*s*v + 2*dd*m*s*w + 2*dd*m*s*y - 4*dd*m*v*z + 2*dd*m*w*z + 2*dd*m*y*z + 4*dd*n*p*v - 4*dd*n*p*w + 8*dd*n*s*v - 4*dd*n*s*w - 4*dd*n*s*y + 8*dd*n*v*z - 4*dd*n*w*z - 4*dd*n*y*z - 2*dd*o*p*v + 2*dd*o*p*w - 4*dd*o*s*v + 2*dd*o*s*w + 2*dd*o*s*y - 4*dd*o*v*z + 2*dd*o*w*z + 2*dd*o*y*z + 4*dd*p*q*x - 2*dd*p*r*x + 4*dd*q*s*u + 4*dd*q*s*x + 4*dd*q*u*z + 4*dd*q*x*z - 2*dd*r*s*u - 2*dd*r*s*x - 2*dd*r*u*z - 2*dd*r*x*z + 2*d*f*g*pp - 2*d*f*h*pp - 6*d*g*pp*q + 4*d*g*pp*r + 8*d*h*pp*q - 6*d*h*pp*r - 2*d*l*pp*q + 2*d*l*pp*r + 4*f*g*pp*s - 8*f*h*pp*s + 4*f*l*pp*s - 8*g*pp*q*s + 4*g*pp*r*s + 16*h*pp*q*s - 8*h*pp*r*s - 8*l*pp*q*s + 4*l*pp*r*s) / den
    a4 = (2*addk - add*u - add*x + a*m*pp - 2*a*n*pp + a*o*pp - 4*bdd*k + 2*bdd*u + 2*bdd*x - 2*b*m*pp + 4*b*n*pp - 2*b*o*pp + 2*cdd*k - cdd*u - cdd*x + c*m*pp - 2*c*n*pp + c*o*pp - 2*dd*f*k + dd*f*u + dd*f*x - 2*dd*g*v + dd*g*w + dd*g*y + 4*dd*h*v - 2*dd*h*w - 2*dd*h*y + 4*dd*k*q - 2*dd*k*r - 2*dd*l*v + dd*l*w + dd*l*y + 2*dd*m*v - dd*m*w - dd*m*y - 4*dd*n*v + 2*dd*n*w + 2*dd*n*y + 2*dd*o*v - dd*o*w - dd*o*y - 2*dd*q*u - 2*dd*q*x + dd*r*u + dd*r*x - f*g*pp + 2*f*h*pp - f*l*pp + 2*g*pp*q - g*pp*r - 4*h*pp*q + 2*h*pp*r + 2*l*pp*q - l*pp*r) / den
    #b_exec_time = time.time()
    #_exec_time = b_exec_time - a_exec_time
    #print(_exec_time)
...
# mr x mr -> P
def test(N, p0_st, q0_st, p1_st, q1_st):
    ...
    for op in op_id:
        ...
        N1 = len(loads(p0_st).exterior.coords) - 1
        N2 = len(loads(p1_st).exterior.coords) - 1
...
# mr x mr -> bool
def test2(N, p0_st, q0_st, p1_st, q1_st):
    op_id = [1]
    ...
    for op in op_id:
        ...
        while k < N:
            ...
            #pred = get_mr_mr_intersections(r0_m_segs, r1_m_segs, initial_state, final_state, operation)
...
# 3. get regions initial and final states w.r.t to each other

initial_state, final_state = get_mr_states(p0_wkt, q0_wkt, p1_wkt, q1_wkt)

# 4. find if regions intersect
...
if initial_state == State.Touch or      \
   ...
   final_state == State.Intersect:
    ...
    pred, exec_time, solver_exec_time, avg, sec, ssec = intersections_tests2(r0_m_segs, r1_m_segs, initial_state, final_state, 1)
...
print(format(exec_time, precision) + ' sec')