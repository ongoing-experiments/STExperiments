...
from shapely.geometry import Point, Polygon, LineString, MultiPolygon, MultiLineString, LineString, GeometryCollection
import shapely.wkt
from shapely.wkt import dumps, loads
from MSeg import MSeg
from math import atan, sqrt
from Utils import distance
from Classes import Point, Interval, QuadBezierCurve, CCurve, MSU, MovingSegment, MPU, MovingPoint
from CG import direction, dir, distance, dir2
from Solver import get_solutions_quartic_dir
...
def get_params():
    ...
    if len(sys.argv) == 4:
        ...
        if filter == "0":
            USE_FILTER_STEP = False

        TYPE_TEST = int(sys.argv[3])
...
#@jit(nopython=True)
@jit(float64[:](float64, float64, float64, int64, int64, float32))
def get_solutions_quad2(a0, a1, a2, itx, ity, eps):
    #global eps
    ...
    while K < N:
        if r[K] >= itx - eps and r[K] <= itx + eps:
            roots.append(itx)
        elif r[K] >= ity - eps and r[K] <= ity + eps:
            roots.append(ity)
        elif r[K] >= itx and r[K] <= ity:
            roots.append(r[K])
...
def quad_roots(a, b, c, w, v, y, s, d, z, p, it):
    ...
    a0 = (ap2*d2 + 2*ap2d*s + ap2*s2 - 2*bp2d*s - bp22*s2 + cp2*s2 - den*w + 2*d2p*v*z - 2*d2p*w*z + 2*d2v*z2 - d2w*z2 - d2y*z2) / den
    a1 = (-2*ap2d - 2*ap2*s + 2*bp2d + 4*bp2*s - 2*cp2*s - 2*d2p*v + 2*d2p*w - 4*d2v*z + 2*d2w*z + 2*d2y*z) / den
    a2 = (ap2 - bp22 + cp2 + 2*d2v - d2w - d2y) / den
...
def get_valid_roots(msu1, msu2, roots, op):
    ...
    if op == 1:
        for t in roots:
            ...
            Cx, Cy = msu2.at_0(t)
            #b_time = time.time()
            #vrcountert += b_time - a_time

            if on_segment3(Ax, Ay, Bx, By, Cx, Cy):
                valid_roots += [t]
    elif op == 2:
        for t in roots:
            #a_time = time.time()
            ...
            if on_segment3(Ax, Ay, Bx, By, Dx, Dy):
                valid_roots += [t]
    elif op == 3:
        for t in roots:
            #a_time = time.time()
            ...
            if on_segment3(Cx, Cy, Dx, Dy, Ax, Ay):
                valid_roots += [t]
    elif op == 4:
        for t in roots:
            #a_time = time.time()
            ...
            if on_segment3(Cx, Cy, Dx, Dy, Bx, By):
                valid_roots += [t]

    return valid_roots

def get_valid_roots2(msu1, msu2, roots, op):
    #global vrcountert
...
def on_segment3(minx, miny, maxx, maxy, rx, ry):

    if minx > maxx:
        if rx < maxx or rx > minx:
            return False
    else:
...
def on_segment2(px, py, qx, qy, rx, ry):
    if min(px, qx) <= rx <= max(px, qx) and min(py, qy) <= ry <= max(py, qy):
        return True

    return False

def on_segment_w_eps(minx, miny, maxx, maxy, rx, ry, epsilon):
...
@jit(boolean(float64, float64, float64, float64, float64, float64))
def on_segment(minx, miny, maxx, maxy, rx, ry):
...
@jit(boolean(float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, int64, int64, float64, float64))
def msegs_intersect_at_t2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, t, i0x, i1x, dt0, dt1):
    t0 = float((t - i0x) / dt0)
    ...
    Ax = op(c0_cp0_x, mt2, c0_cp1_x, mtt, c0_cp2_x, t2)
    Ay = op(c0_cp0_y, mt2, c0_cp1_y, mtt, c0_cp2_y, t2)

    Bx = op(c1_cp0_x, mt2, c1_cp1_x, mtt, c1_cp2_x, t2)
    By = op(c1_cp0_y, mt2, c1_cp1_y, mtt, c1_cp2_y, t2)
    ...
    Cx = op(c2_cp0_x, mt2, c2_cp1_x, mtt, c2_cp2_x, t2)
    Cy = op(c2_cp0_y, mt2, c2_cp1_y, mtt, c2_cp2_y, t2)

    Dx = op(c3_cp0_x, mt2, c3_cp1_x, mtt, c3_cp2_x, t2)
    Dy = op(c3_cp0_y, mt2, c3_cp1_y, mtt, c3_cp2_y, t2)
    
    d1 = dir2(Ax, Ay, Bx, By, Cx, Cy)
    d2 = dir2(Ax, Ay, Bx, By, Dx, Dy)

    if (d1 * d2) > 0:
        return False

    d3 = dir2(Cx, Cy, Dx, Dy, Ax, Ay)
    d4 = dir2(Cx, Cy, Dx, Dy, Bx, By)

    return d3 * d4 < 0    

@jit(float64[3](float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64))
def quad_coeff(a, b, c, d, o, f, g, h, k, l, m, n, p, q, r, s):
    q2 = q*q
    p2 = p*p
    den = q2*s
    anq2 = a*n*q2
    ...
    a0 = (a*g*q2*s - ahpq*s - a*m*q2*s + anq2*r - bg*pqs + bh*p2*s + bmpq*s - bnpq*r + c*f*pqs + c*k*q2*s - c*l*q2*r - c*o*q2*s - dfs*p2 - d*k*pqs + dl*pq*r + d*o*pqs - f*m*pqs + fn*pq*r - g*k*q2*s + g*l*q2*r + h*k*pqs - hlq*p*r + m*o*q2*s - noq2*r ) / den
    a1 = (ahq*s - anq2 + bg*q*s - 2*bh*p*s - bmq*s + bnpq + bnq*r - c*f*q*s + c*l*q2 + 2*dfs*p + d*k*q*s - dl*pq - dl*q*r - d*o*q*s + f*m*q*s - fn*pq - fn*q*r - g*l*q2 - h*k*q*s + h*l*pq + hlq*r + noq2 ) / den
    a2 = (bh*s - bnq - dfs + dl*q + fn*q - hlq ) / den

    return (a0, a1, a2)

def quad_roots4(a, b, c, d, o, f, g, h, k, l, m, n, p, q, r, s, it, eps):
    #global eps
    #s_exec_time = time.time()
    ...
    a0 = (a*g*q2*s - a*h*p*q*s - a*m*q2*s + a*n*q2*r - b*g*p*q*s + b*h*p2*s + b*m*p*q*s - b*n*p*q*r + c*f*p*q*s + c*k*q2*s - c*l*q2*r - c*o*q2*s - d*f*p2*s - d*k*p*q*s + d*l*p*q*r + d*o*p*q*s - f*m*p*q*s + f*n*p*q*r - g*k*q2*s + g*l*q2*r + h*k*p*q*s - h*l*p*q*r + m*o*q2*s - n*o*q2*r ) / den
    a1 = (a*h*q*s - a*n*q2 + b*g*q*s - 2*b*h*p*s - b*m*q*s + b*n*p*q + b*n*q*r - c*f*q*s + c*l*q2 + 2*d*f*p*s + d*k*q*s - d*l*p*q - d*l*q*r - d*o*q*s + f*m*q*s - f*n*p*q - f*n*q*r - g*l*q2 - h*k*q*s + h*l*p*q + h*l*q*r + n*o*q2 ) / den
    a2 = (b*h*s - b*n*q - d*f*s + d*l*q + f*n*q - h*l*q ) / den
    """
    ...
    r = get_solutions_quad2(a0, a1, a2, it.x, it.y, eps)
    """
    
    coeff = quad_coeff(a, b, c, d, o, f, g, h, k, l, m, n, p, q, r, s)
    #r = get_solutions_quad2(coeff[0], coeff[1], coeff[2], it.x, it.y, eps)

    #e_exec_time = time.time()
    #exec_time = e_exec_time - s_exec_time

    #return exec_time, r
    return get_solutions_quad2(coeff[0], coeff[1], coeff[2], it.x, it.y, eps)
...
def msegs_intersect_at_midpoint(msu1, msu2, t1, t2):
    return msegs_intersect_at_t(msu1, msu2, (t1 + t2) / 2)
    """
    t = (t1 + t2) / 2
...
def get_int2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, msu1_t0, msu2_t0, msu1_dt, msu2_dt, it0, r1, msegs_intersect_at_t0):                      
    #r = []
    ...
    if msegs_intersect_at_t0:
        if it0.x == r1:
            #if msegs_intersect_at_midpoint(msu1, msu2, r1[0], it0.y):
            #if msegs_intersect_at_t2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, (r1[0] + it0.y) / 2, msu1_t0, msu2_t0, msu1_dt, msu2_dt):
            #    I = Interval(r1[0], it0.y, True, True)
            #    r = [I]
            #else:
            return [r1]
            ...
            #I = Interval(it0.x, r1[0], True, True)
            #r = [I]
            return [Interval(it0.x, r1, True, True)]
    else:
        # msegs intersect.
        if r1 == it0.y:
            return [r1]
        #elif msegs_intersect_at_midpoint(msu1, msu2, r1, it0.y):
        elif msegs_intersect_at_t2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, (r1 + it0.y) / 2, msu1_t0, msu2_t0, msu1_dt, msu2_dt):
            #I = Interval(r1[0], it0.y, True, True)
            return [Interval(r1, it0.y, True, True)]
        # 2.2 collinear.
        #elif (d1 * d2) == 0 or (d3 * d4) == 0:
		#   ...
        # 2.3 touch.
...
def get_msegs_intersections2(ms1, ms2, i):
    global solver_exec_time
    global solver_exec_times
    global osolver_exec_time
    global osolver_exec_times
    global epsilon
    global USE_FILTER_STEP
    global eps
        ...
        if it != None:
            ...
            while idx1 < n_it_ms1 and idx2 < n_it_ms2:
                ...
                if it0 != None:
                    ...
                    if USE_FILTER_STEP:
                        flag = not (msu1.left > msu2.right or msu1.right < msu2.left or msu1.top < msu2.bottom or msu1.bottom > msu2.top)
                        if not flag:
                            col = 1

                        if flag:
                            q = msu1.c0.intervals[idx1].y - msu1.c0.intervals[idx1].x
                            p = msu1.c0.intervals[idx1].x
                            #T = (t - msu1.c0.intervals[idx1].x) / dt
                            #Ax, Ay, Bx, By = msu1.atop(T)
                            a, b, c, d, o, f, g, h = msu1.atop2()

                            s1 = msu2.c0.intervals[idx1].y - msu2.c0.intervals[idx1].x
                            r1 = msu2.c0.intervals[idx1].x
                            #T = (t - msu2.c0.intervals[idx1].x) / dt
                            k, l, m, n, t, u, v, z = msu2.atop2()
                            #Cx, Cy, Dx, Dy = msu2.atop(T)
                            ...
                            #exec_time, r01 = get_solutions(f, t, it)
                            r01 = quad_roots4(a, b, c, d, o, f, g, h, k, l, m, n, p, q, r1, s1, it, eps)
                            #osolver_exec_time += exec_time

                            #f = (((Bx - Ax) * (Dy - Ay)) - ((By - Ay) * (Dx - Ax)))
                            #exec_time, r02 = get_solutions(f, t, it)
                            r02 = quad_roots4(a, b, c, d, o, f, g, h, t, u, v, z, p, q, r1, s1, it, eps)					
                            #osolver_exec_time += exec_time

                            #f = (((Dx - Cx) * (Ay - Cy)) - ((Dy - Cy) * (Ax - Cx)))
                            #exec_time, r03 = get_solutions(f, t, it)
                            r03 = quad_roots4(k, l, m, n, t, u, v, z, a, b, c, d, p, q, r1, s1, it, eps)					
                            #osolver_exec_time += exec_time

                            #f = (((Dx - Cx) * (By - Cy)) - ((Dy - Cy) * (Bx - Cx)))
                            #exec_time, r04 = get_solutions(f, t, it)
                            r04 = quad_roots4(k, l, m, n, t, u, v, z, o, f, g, h, p, q, r1, s1, it, eps)					
                            #osolver_exec_time += exec_time

                            r01 = get_valid_roots(msu1, msu2, r01, 1)
                            r02 = get_valid_roots(msu1, msu2, r02, 2)
                            r03 = get_valid_roots(msu1, msu2, r03, 3)
                            r04 = get_valid_roots(msu1, msu2, r04, 4)

                            osolver_exec_times += 4

                            if len(r01) == 0 and len(r02) == 0 and len(r03) == 0 and len(r04) == 0:
                                col = 1
                    ...
                    if col == 0:
                        ...
                        #f = (((Bx - Ax) * (Cy - Ay)) - ((By - Ay) * (Cx - Ax)))
                        exec_time, r1 = get_solutions_quartic_dir(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y, c2_cp2_y, msu1_t0, msu1_dt, msu2_t0, msu2_dt, it, eps)
                        solver_exec_times += 1
                        ...
                        #f = (((Bx - Ax) * (Dy - Ay)) - ((By - Ay) * (Dx - Ax)))
                        exec_time, r2 = get_solutions_quartic_dir(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y, c3_cp2_y, msu1_t0, msu1_dt, msu2_t0, msu2_dt, it, eps)
                        solver_exec_times += 1
                        ...
                        #f = (((Dx - Cx) * (Ay - Cy)) - ((Dy - Cy) * (Ax - Cx)))
                        exec_time, r3 = get_solutions_quartic_dir(c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y, c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y, c3_cp2_y, c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, msu1_t0, msu1_dt, msu2_t0, msu2_dt, it, eps)
                        solver_exec_times += 1
                        ...
                        #f = (((Dx - Cx) * (By - Cy)) - ((Dy - Cy) * (Bx - Cx)))
                        exec_time, r4 = get_solutions_quartic_dir(c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y, c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y, c3_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, msu1_t0, msu1_dt, msu2_t0, msu2_dt, it, eps)
                        solver_exec_times += 1
...
def op(a, b, c, d, e, f):
    return a * b + 2 * c * d + e * f

@jit(boolean(float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, int64, int64, float64, float64))
def v_root(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, t, i0x, i1x, dt0, dt1):
    t0 = float((t - i0x) / dt0)
    ...
    #print(mt, mt2, mtt, t2, t0)
    #print(c0_cp0_x, c0_cp1_x, c0_cp2_x)
    ...
    return on_segment(Ax, Ay, Bx, By, Cx, Cy)
                ...
                if it0 != None:
                    ...
                    if USE_FILTER_STEP:
                        ...
                        if flag:
                            ...
                            #q = msu1.c0.intervals[idx1].y - msu1.c0.intervals[idx1].x      #msu1_dt
                            #p = msu1.c0.intervals[idx1].x                                                 #msu1_t0

                            a, b, c, d, o, f, g, h = msu1.atop2()

                            #s1 = msu2.c0.intervals[idx2].y - msu2.c0.intervals[idx1].x     msu2_dt
                            #r1 = msu2.c0.intervals[idx2].x
                            ...
                            r01 = quad_roots4(a, b, c, d, o, f, g, h, k, l, m, n, msu1_t0, msu1_dt, msu2_t0, msu2_dt, it, eps)
                            #exec_time, r01 = quad_roots4(a, b, c, d, o, f, g, h, k, l, m, n, msu1_t0, msu1_dt, msu2_t0, msu2_dt, it, eps)
                            #osolver_exec_time += exec_time

                            r02 = quad_roots4(a, b, c, d, o, f, g, h, t, u, v, z, msu1_t0, msu1_dt, msu2_t0, msu2_dt, it, eps)					
                            #exec_time, r02 = quad_roots4(a, b, c, d, o, f, g, h, t, u, v, z, msu1_t0, msu1_dt, msu2_t0, msu2_dt, it, eps)					
                            #osolver_exec_time += exec_time

                            r03 = quad_roots4(k, l, m, n, t, u, v, z, a, b, c, d, msu1_t0, msu1_dt, msu2_t0, msu2_dt, it, eps)					
                            #exec_time, r03 = quad_roots4(k, l, m, n, t, u, v, z, a, b, c, d, msu1_t0, msu1_dt, msu2_t0, msu2_dt, it, eps)					
                            #osolver_exec_time += exec_time

                            r04 = quad_roots4(k, l, m, n, t, u, v, z, o, f, g, h, msu1_t0, msu1_dt, msu2_t0, msu2_dt, it, eps)					
                            #exec_time, r04 = quad_roots4(k, l, m, n, t, u, v, z, o, f, g, h, msu1_t0, msu1_dt, msu2_t0, msu2_dt, it, eps)					
                            #osolver_exec_time += exec_time

                            osolver_exec_times += 4

                            b_time = time.time()
                            appcountert += b_time - a_time
                            ...
                            if len(r01) > 0:
                                ...
                                for t in r01:
                                    if v_root(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, float(t), msu1_t0, msu2_t0, msu1_dt, msu2_dt):
                                        valid_roots += [t]
                            ...
                            if len(r02) > 0:
                                ...
                                for t in r02:
                                    if v_root(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, float(t), msu1_t0, msu2_t0, msu1_dt, msu2_dt):
                                        valid_roots += [t]
                            ...
                            if len(r03) > 0:
                                ...
                                for t in r03:
                                    if v_root(c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y, c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y, c3_cp2_y, c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y,  c0_cp2_y, float(t), msu2_t0, msu1_t0, msu2_dt, msu1_dt):
                                        valid_roots += [t]
                            ...
                            if len(r04) > 0:
                                ...
                                for t in r04:
                                    if v_root(c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y, c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y, c3_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y,  c1_cp2_y, float(t), msu2_t0, msu1_t0, msu2_dt, msu1_dt):
                                        valid_roots += [t]
                    ...
                    if col == 0:
                        ...
                        msegs_intersect_at_t0 = msegs_intersect_at_t2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, it0.x, msu1_t0, msu2_t0, msu1_dt, msu2_dt)
                        ...
                        if n1 == 1 and n2 == 1 and n3 == 0 and n4 == 0:
                            # 1.1 msegs intersect.
                            #if msegs_intersect_at_midpoint(msu1, msu2, r1[0], r2[0]):
                            if msegs_intersect_at_t2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, (r1[0] + r2[0]) / 2, msu1_t0, msu2_t0, msu1_dt, msu2_dt):
                            ...
                            # 1.3 touch.
                                ...
                                r.sort()
                        # 1. both endpoints of one of the segments intersect the other segment
                        elif n1 == 0 and n2 == 0 and n3 == 1 and n4 == 1:
                            # 1.1 msegs intersect.
                            #if msegs_intersect_at_midpoint(msu1, msu2, r3[0], r4[0]):
                            if msegs_intersect_at_t2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, (r3[0] + r4[0]) / 2, msu1_t0, msu2_t0, msu1_dt, msu2_dt):
                        ...
                        elif n1 == 1 and n2 == 0 and n3 == 0 and n4 == 0:
                            #r = get_int(msu1, msu2, it0, r1, msegs_intersect_at_t0)

                            r = get_int2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, msu1_t0, msu2_t0, msu1_dt, msu2_dt, it0, r1[0], msegs_intersect_at_t0)
                        # 2. an endpoint of one of the segments intersects the other segment
                        elif n1 == 0 and n2 == 1 and n3 == 0 and n4 == 0:
                            #r = get_int(msu1, msu2, it0, r2, msegs_intersect_at_t0)
                            
                            r = get_int2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, msu1_t0, msu2_t0, msu1_dt, msu2_dt, it0, r2[0], msegs_intersect_at_t0)
                        # 2. an endpoint of one of the segments intersects the other segment
                        elif n1 == 0 and n2 == 0 and n3 == 1 and n4 == 0:
                            #r = get_int(msu1, msu2, it0, r3, msegs_intersect_at_t0)

                            r = get_int2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, msu1_t0, msu2_t0, msu1_dt, msu2_dt, it0, r3[0], msegs_intersect_at_t0)
                        # 2. an endpoint of one of the segments intersects the other segment
                        elif n1 == 0 and n2 == 0 and n3 == 0 and n4 == 1:
                            #r = get_int(msu1, msu2, it0, r4, msegs_intersect_at_t0)

                            r = get_int2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, msu1_t0, msu2_t0, msu1_dt, msu2_dt, it0, r4[0], msegs_intersect_at_t0)
                        # 3. an endpoint of both segments intersects the other segment
                        elif n1 == 1 and n2 == 0 and n3 == 1 and n4 == 0:
                            # 3.1 msegs intersect.
                            #if msegs_intersect_at_midpoint(msu1, msu2, r1[0], r3[0]):
                            if msegs_intersect_at_t2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, (r1[0] + r3[0]) / 2, msu1_t0, msu2_t0, msu1_dt, msu2_dt):
                            ...
                            # 3.3 touch.
                                ...
                                r.sort()
                        # 3. an endpoint of both segments intersects the other segment
                        elif n1 == 1 and n2 == 0 and n3 == 0 and n4 == 1:
                            # 3.1 msegs intersect.
                            #if msegs_intersect_at_midpoint(msu1, msu2, r1[0], r4[0]):
                            if msegs_intersect_at_t2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, (r1[0] + r4[0]) / 2, msu1_t0, msu2_t0, msu1_dt, msu2_dt):
                            ...
                            # 3.3 touch.
                                ...
                                r.sort()
                        # 3. an endpoint of both segments intersects the other segment
                        elif n1 == 0 and n2 == 1 and n3 == 1 and n4 == 0:
                            # 3.1 msegs intersect.
                            #if msegs_intersect_at_midpoint(msu1, msu2, r2[0], r3[0]):
                            if msegs_intersect_at_t2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, (r2[0] + r3[0]) / 2, msu1_t0, msu2_t0, msu1_dt, msu2_dt):
                            ...
                            # 3.3 touch.
                                ...
                                r.sort()
                        # 3. an endpoint of both segments intersects the other segment
                        elif n1 == 0 and n2 == 1 and n3 == 0 and n4 == 1:
                            # 3.1 msegs intersect.
                            #if msegs_intersect_at_midpoint(msu1, msu2, r2[0], r4[0]):
                            if msegs_intersect_at_t2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, (r2[0] + r4[0]) / 2, msu1_t0, msu2_t0, msu1_dt, msu2_dt):
                        ...
                        # 4. an endpoint of a segment intersects the other segment twice.
                        elif n1 == 2 and n2 == 0 and n3 == 0 and n4 == 0:
                            # 4.1 msegs intersect.
                            #if msegs_intersect_at_midpoint(msu1, msu2, r1[0], r1[1]):
                            if msegs_intersect_at_t2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, (r1[0] + r1[1]) / 2, msu1_t0, msu2_t0, msu1_dt, msu2_dt):
                            ...
                            #print_roots(r)
                        # 4. an endpoint of a segment intersects the other segment twice.
                        elif n1 == 0 and n2 == 2 and n3 == 0 and n4 == 0:
                            # 4.1 msegs intersect.
                            #if msegs_intersect_at_midpoint(msu1, msu2, r2[0], r2[1]):
                            if msegs_intersect_at_t2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, (r2[0] + r2[1]) / 2, msu1_t0, msu2_t0, msu1_dt, msu2_dt):
                            ...
                            # 4.3 touch.
                                ...
                                r.sort()
                        # 4. an endpoint of a segment intersects the other segment twice.
                        elif n1 == 0 and n2 == 0 and n3 == 2 and n4 == 0:
                            # 4.1 msegs intersect.
                            #if msegs_intersect_at_midpoint(msu1, msu2, r3[0], r3[1]):
                            if msegs_intersect_at_t2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, (r3[0] + r3[1]) / 2, msu1_t0, msu2_t0, msu1_dt, msu2_dt):
                            ...
                            # 4.3 touch.
                                ...
                                r.sort()
                        # 4. an endpoint of a segment intersects the other segment twice.
                        elif n1 == 0 and n2 == 0 and n3 == 0 and n4 == 2:
                            # 4.1 msegs intersect.
                            #if msegs_intersect_at_midpoint(msu1, msu2, r4[0], r4[1]):
                            if msegs_intersect_at_t2(c0_cp0_x, c0_cp1_x, c0_cp2_x, c0_cp0_y, c0_cp1_y, c0_cp2_y, c1_cp0_x, c1_cp1_x, c1_cp2_x, c1_cp0_y, c1_cp1_y, c1_cp2_y, c2_cp0_x, c2_cp1_x, c2_cp2_x, c2_cp0_y, c2_cp1_y,  c2_cp2_y, c3_cp0_x, c3_cp1_x, c3_cp2_x, c3_cp0_y, c3_cp1_y,  c3_cp2_y, (r4[0] + r4[1]) / 2, msu1_t0, msu2_t0, msu1_dt, msu2_dt):
...
def intersections_tests2(MS0, MS1, initial_state, final_state, op = 1):
    ...
    for ms0 in msegs0:
        for ms1 in msegs1:
            if get_msegs_intersections2(ms0, ms1, interval):
                ...
                return True, exec_time, solver_exec_time, ((solver_exec_time * 100) / exec_time), seconds, sseconds
    ...
    return False, exec_time, solver_exec_time, ((solver_exec_time * 100) / exec_time), seconds, sseconds
...
def intersections_tests(MS0, MS1, initial_state, final_state, op = 1):
    ...
    #print(pcounter, fpcounter, aabbfcounter, appcounter, len(intersections))
    #print(fpcountert, aabbfcountert, appcountert, pcountert, ecountert, coefft, vrcountert, ccc)
    #print_period(intersections)
    #print('')
...
def test(N, p0_wkt, q0_wkt, p1_wkt, q1_wkt):
    op_id = [1, 2, 3, 4, 6, 7, 8, 9]
    op_id = [1]
    ...
    for op in op_id:
        ...
        while k < N:
            ...
            r0_m_segs = get_moving_segs_from_observations(p0, q0, t0, t1)

            if r0_m_segs == None:
                print_error(err_code, err_msg)

            r1_m_segs = get_moving_segs_from_observations(p1, q1, t0, t1)

            if r1_m_segs == None:
                print_error(err_code, err_msg)

            initial_state, final_state = get_mr_states(p0, p1, q0, q1)
        ...
        N1 = len(loads(p0_wkt).exterior.coords) - 1
        N2 = len(loads(p1_wkt).exterior.coords) - 1

        t_avg = t_avg / N
        res += [(format(min_exec_time, precision3), format(max_exec_time, precision3), format(min_solver_exec_time, precision3), format(max_solver_exec_time, precision3), format(t_avg, precision), str(solver_exec_times), str(osolver_exec_times), str(N1), str(N2))]

    return res

# mr x mr -> bool

def test2(N, p0_wkt, q0_wkt, p1_wkt, q1_wkt):
    #op_id = [1, 2, 3, 4, 6, 7, 8, 9]
    op_id = [1]
    ...
    for op in op_id:
        ...
        while k < N:
            ...
            if initial_state == State.Touch or      \
               initial_state == State.Inside or     \
               initial_state == State.Intersect or  \
               final_state == State.Touch or        \
               final_state == State.Inside or       \
               final_state == State.Intersect:
                ...
                pred, exec_time, solver_exec_time, avg, sec, ssec = intersections_tests2(r0_m_segs, r1_m_segs, initial_state, final_state, op)
        ...
        t_avg = t_avg / N
        res += [(format(min_exec_time, precision3), format(max_exec_time, precision3), format(min_solver_exec_time, precision3), format(max_solver_exec_time, precision3), format(t_avg, precision), str(solver_exec_times), str(osolver_exec_times), str(pred), str(N1), str(N2))]
...
if TESTING:
    ...
    p0 = 'POLYGON((975.0420063220051 697.090167065809, 968.2376237623762 719.366754617414, 949.8141049487542 719.682075626039, 947.5206593693675 726.5578738835004, 929.1730947342762 738.0175376459358, 929.5007298170459 743.256241080192, 924.3364137823626 741.690195051915, 919.6716773339609 738.3449566105769, 912.7913405958017 741.619146256987, 901.9793828644084 732.4514152470385, 891.8226952985542 721.3191704492442, 884.6147234776257 716.73530494427, 877.4067516566968 712.1514394392957, 873.1474955806933 721
    q = 'Polygon ((1020.93312994072357469 663.84998742016637152, 1019.3785180249685709 660.77526998277312487, 1022.58823706176826818 657.53116085866645335, 1026.08538695465472301 656.63540865758068321, 1029.51503604783761148 656.58440506067063325, 1032.77575868154735872 656.7699450297468502, 1035.64809002664105719 658.99979755615458998, 1037.77640058646852594 654.26904254323608257, 1036.84672352531629258 649.31893900798490904, 1035.35949610245415897 644.14925433678035915, 1035.68015397774706798 640.44925312435452724, 1037.62269195487829165 636.47878348587528308, 1039.64992577536600038 634.89047932367361682, 1043.8231304265750623 636.79920874754816396, 1046.72927032492520993 639.23179628668549412, 1049.01070798229284264 646.04020718295146253, 1048.25112044871389116 653.3557463780668968, 1048.40354970539374335 657.39360050943923852, 1053.40454053074518015 658.64335391183431057, 1059.03046621069916 658.01772121637168311, 1062.49380755032871093 656.91923400255586785, 1066.10922922620807185 656.10794503120678201, 1068.10300341386937362 658.06756174518113767, 1069.48857256334122212 660.12860411892552293, 1068.13716088755609235 662.02095264681702247, 1065.82284513141212301 665.01155577490408177, 1061.12613128537032026 665.58641748707668739, 1057.08804454037817777 663.23840948448605559, 1052.54329472377639831 662.85025080948980758, 1048.06639462730777268 665.36799941922390644, 1046.07308566688675455 668.40925722379074614, 1047.74608567244490587 672.81865652055603277, 1052.51076554542987651 676.39992072274844759, 1058.82959210692979468 678.05502784379336845, 1061.78627037495016339 678.91638995757239172, 1066.61856735444825972 682.90312418522466942, 1067.07504097681794519 686.26515617189443219, 1064.77757134389730709 688.73201749152849516, 1062.47998540416620017 689.94866018152697507, 1058.57713287220053644 688.41159222985561428, 1056.32985268851962246 685.5565722352253033, 1051.63255730842774938 679.88034079922158526, 1047.78024314613185197 676.77204742219191758, 1044.18143497985556678 674.55915732581752309, 1041.05583025236205685 673.9343387780249941, 1042.64448333499422006 679.7122284874185425, 1044.18236543433590668 684.5609063628999138, 1045.14585104874504395 689.71374491088113245, 1043.64259165033013232 693.81933245557684131, 1039.46985222636135404 696.91147755024348953, 1034.02993174272387478 699.27726213053801985, 1030.66766714243408387 697.23329849363699395, 1026.71415993398841238 696.01723733768812963, 1023.04761835438876005 692.14865858621897132, 1022.48983537905928642 689.42864019099533834, 1027.25381741118417267 685.50859261537607381, 1029.99044931601110875 681.9266305723235746, 1032.76065716047469323 676.04696628272949965, 1032.55734061369480514 669.82968168780962515, 1031.40831503020922355 667.93756577353815373, 1030.22559720027720687 667.09293347617244763, 1027.55623348153335428 667.32970965577897005, 1024.04200485180376745 666.24876640604702516, 1020.93312994072357469 663.84998742016637152))'
    ...
    if TYPE_TEST == 0:
        print('Mr X Mr -> P : N Tests: ' + str(NTESTS) + ' : Filter Step: ' + str(USE_FILTER_STEP))
        print('Op;mET (sec);MET (sec);mSET (sec);MSET (sec);AVGET %;NE;ONE;NV1;NV2;')
        op_st = ['Intersects', 'Touches', 'Equals', 'Disjoint', 'Within', 'Overlaps', 'CoveredBy', 'Covers']
        op_st = ['Intersects']
    else:
        print('Mr X Mr -> Bool : N Tests: ' + str(NTESTS) + ' : Filter Step: ' + str(USE_FILTER_STEP))
        print('Op;mET (sec);MET (sec);mSET (sec);MSET (sec);AVGET %;NE;ONE;R;NV1;NV2;')
        op_st = ['Intersects']
...
if operation == Operation.Intersects.value or \
   ...
   operation == Operation.Overlaps.value:
    ...
    print_error(-1, 'Unsupported operation:: ' + str(operation) + '!')
