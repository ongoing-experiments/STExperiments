...
class MovingSegment:
    ...
    def wkt_at(self, t):
        ...
        """
        # 1947.13510898305, 1947.13510898305
        if t > 1947 and t < 1947.136:
            #dec = decimal.Decimal(t)
            print(format(t, '.60g'), t == 1947.13510898305, t > 1947.13510898305, t - 1947.13510898305)
            sys.exit()
        """
...
class Period:
    ...
    def union(self, p2):
        ...
        while j1 < N1 and j2 < N2:
            ...
            if isinstance(t1, Interval) and isinstance(t2, Interval):
                ...
                if T == None:
                    ...
                    a0 = min(t1.begin(), t2.begin())
                    a1 = max(t1.end(), t2.end())

                    union += [Interval(a0, a1, True, True)]
...
#-------------------------------------------------------------------
def create_moving_segment(units, pass_through_control_points, _units = None):
    ms = MovingSegment()
...
def get_samples_for_out2(MS, mp1, mp2, i, n_samples, s1, s2):
    n = (n_samples - 1)
    k = 0
    dt = i.y - i.x
    eq = False
    ...
    for j in range(0, n_samples):
        ...
        if len(s1) > 0 and k < len(s1):
            if isinstance(s1[k], Interval):
                if s1[k].contains(t):
                    print(mline.wkt)
                    #print(mp1.wkt_at(t))
                    x1, y1 = mp1.at(t)
                    x2, y2 = mp2.at(t)
                    print(LineString([(x1, y1), (x2, y2)]).wkt)
...
def get_samples_for_out3(MS, ms, mp1, mp2, i, n_samples, s):
    n = (n_samples - 1)
    k = 0
    dt = i.y - i.x
    eq = False
...
def get_samples_for_out4(MS, ms, mp1, mp2, i, n_samples, s):
    n = (n_samples - 1)
    k = 0
    dt = i.y - i.x
    eq = False
    ...
    for j in range(0, n_samples):
        ...
        if len(s) > 0 and k < len(s):
            if isinstance(s[k], Interval):
                if s[k].contains(t):
                    ...
                    """
                    if ms.wkt_at(t) == 'LINESTRING EMPTY':
                        u = ms.units[2]
                        u = u.i
                        print(t, u.y, t > u.y)
                        sys.exit()
        ...
        # 1947.13510898305
        """
        if out == 1 and ms.wkt_at(t) == 'LINESTRING EMPTY':
            print(t, ms.to_string())
            sys.exit()
        """
...
def get_intersection_information(intersection_instants, msegs, mp, op):
    ...
    if op == Operation.Intersects.value:
        ...
        info = "Contains: "
    elif op == Operation.Intersection.value:
        info = "Intersection: "
...
def get_m_linestring_at(msegs, t):
...
def get_mr_mp_intersections2(MS, mp, initial_state, final_state, op = 1, p_linear_traj = False):
    ...
    return intersections, msegs

def test_mr_ms_int():
    it = [Interval(1283.57, 1439.63, True, True), Interval(1439.63, 1806.04, True, True), Interval(1806.04, 1947.14, True, True)]

    it1 = Interval(1283.57, 1439.63, True, False)
    it2 = Interval(1439.63, 1806.04, True, False)
    it3 = Interval(1806.04, 1947.14, True, False)
    ...
    l = LineString([(x3, y3), (x4, y4)])
    r = get_mline(msegs1, t)
    s_it = r.intersection(l)

    if s_it.geom_type != 'Point':
        print_error(-1, 'Intersection is not a Point. TODO.')
    ...
    unit1 = str(x1) + ',' + str(y1) + ',' + str(x3) + ',' + str(y3) + ',' + str(x5) + ',' + str(y5) + '#'
    unit1 += str(x2) + ',' + str(y2) + ',' + str(x4) + ',' + str(y4) + ',' + str(x6) + ',' + str(y6) + '#'
    unit1 += str(it1.x) + ',' + str(it1.y) + ':' + str(it1.x) + ',' + str(it1.y)
    ...
    unit2 = str(x1) + ',' + str(y1) + ',' + str(x3) + ',' + str(y3) + ',' + str(x5) + ',' + str(y5) + '#'
    unit2 += str(x2) + ',' + str(y2) + ',' + str(x4) + ',' + str(y4) + ',' + str(x6) + ',' + str(y6) + '#'
    unit2 += str(it2.x) + ',' + str(it2.y) + ':' + str(it2.x) + ',' + str(it2.y)
    ...
    unit3 = str(x1) + ',' + str(y1) + ',' + str(x3) + ',' + str(y3) + ',' + str(x5) + ',' + str(y5) + '#'
    unit3 += str(x2) + ',' + str(y2) + ',' + str(x4) + ',' + str(y4) + ',' + str(x6) + ',' + str(y6) + '#'
    unit3 += str(it3.x) + ',' + str(it3.y) + ':' + str(it3.x) + ',' + str(it3.y)

    units = [unit1, unit2, unit3]
    return units

def get_unit_first_mp_inside(mpoint1, mpoint2, i):
    t = i.begin()
    ...
    _unit = [[[x1, y1, x3, y3, x5, y5], [x2, y2, x4, y4, x6, y6], uinterval.begin(), uinterval.end()], [uinterval.begin(), uinterval.end()]]

    return unit, _unit

def get_unit_second_mp_inside(mpoint1, mpoint2, i):
    t = i.begin()
...
def get_unit_ms_inside(mpoint1, mpoint2, i):
    t = i.begin()
...
"""
	...
	Point (695.86019429682539794 589.18810308210254334)
	Point (806.89610697430214259 654.29816706051815345)
	Point (866.77411224016657343 758.35800145459324995)
	
	Point (745.27408213758735656 511.28856225078374109)
	Point (878.40090937916943403 601.39624007805537076)
	Point (938.86025450198394537 706.61875418603085564)
	
	695.86,589.19#806.89,654.29#866.77,758.36#1000,2000
	745.27,511.29#878.40,601.39#938.86,706.62#1000,2000
		
	Usage examples:
		python mr_ms_pred.py "POLYGON((975.0420063220051 697.090167065809, 968.2376237623762 719.366754617414, 949.8141049487542 719.682075626039, 947.5206593693675 726.5578738835004, 929.1730947342762 738.0175376459358, 929.5007298170459 743.256241080192, 924.3364137823626 741.690195051915, 919.6716773339609 738.3449566105769, 912.7913405958017 741.619146256987, 901.9793828644084 732.4514152470385, 891.8226952985542 721.3191704492442, 884.6147234776257 716.73530494427, 877.4067516566968 712.1514394392957, 873.1474955806933 721.3191704492442, 868.8882395046899 730.4869014591925, 851.6435643564355 749.7625329815303, 845.7678029771724 751.5623397481193, 839.0734469726663 758.972351382961, 829.2443944895814 769.7771772161138, 823.6745980825001 760.609446206166, 814 751.4999999999998, 811.0891089108911 744.6965699208442, 802.3783177024834 736.3804428227309, 798 736,
1289672690934, 735.4026288700353 597.7088326122334, 738 589, 763.0621077701443 576.5999880779154, 814 568, 845.6261486280558 565.4677432801209, 852 570, 878.7172919877744 575.2903122193516, 892.7049504950495 586.1319261213721, 901.829702970297 592.211081794195, 907.3811614420508 597.0972549398289, 928 614.0000000000002, 975.0420063220051 697.090167065809))" "POLYGON ((965 635, 963.1683168316831 648.4432717678101, 958.0990099009902 658.5751978891822, 947.960396039604 658.5751978891822, 942 675, 932 679, 933.5 683.9999999999998, 935 689, 919.2997169901681 692.9781016562131, 907.4059405940594 688.9709762532982, 897.2673267326733 683.9050131926122, 884.6074998412831 681.6848638401293, 875 680, 875.5385318320745 694.8658276536648, 871 710, 861.6493140294922 728.7013719410154, 858.3483767115072 735.028033248251, 855.0474393935223 741.3546945554867, 851.6115528967149 747.7221555823205, 837.1294340674169 742.0589775899646, 826.2970297029703 734.5646437994724, 816.1584158415842 729.4986807387863, 810.6838257704394 731.3618636044034, 793.0534202391211 721.608612617568, 772.9043853461864 709.6530146337057, 755.3267326732673 704.1688654353561, 741 698, 724.9108910891089 688.9709762532982, 704.6336633663364 668.707124010554, 703 651.0000000000002, 702.6975918911147 637.9194267305304, 705 626, 714.7722772277225 602.8496042216359, 760 570, 795.8811881188119 552.1899736147757, 834.2959760355977 574.0513638167383, 846.5742574257424 562.3218997361478, 878 573.0000000000002, 887.1287128712871 572.4538258575199, 902.3366336633663 582.5857519788917, 950.7825840103793 618.7275457564351, 965 635))" "695.86,589.19#806.89,654.29#866.77,758.36#1000,2000" "745.27,511.29#878.40,601.39#938.86,706.62#1000,2000" "1" "1" "1" "100" "1000,2000" "0.0000001" "2" "1"
...
m1 = (y2 - y1 + t *(dy2 - dy1)) / (x2 - x1 + t *(dx2 - dx1))
b1 = y1 + dy1 * t - m1 * (x1 + dx1 * t)
...
m2 = (y4 - y3 + t *(dy4 - dy3)) / (x4 - x3 + t *(dx4 - dx3))
b2 = y3 + dy3 * t - m2 * (x3 + dx3 * t)
...
i1, msegs1 = get_mr_mp_intersections2(reg_m_segs, mpoint1, initial_state, final_state, operation, p_linear_traj)

initial_state, final_state = get_states(p_wkt, q_wkt, mpoint2)
mpoint2 = create_moving_point([mpoint2])

i2, msegs2 = get_mr_mp_intersections2(reg_m_segs, mpoint2, initial_state, final_state, operation, p_linear_traj)

#get_samples_for_out2(msegs1, mpoint1, mpoint2, interval, n_samples, i1, i2)

#if print_intersection_information:
#    print(get_intersection_information(i1, msegs1, mpoint1, operation))

#Intersects: [1283.57, 1806.04]
#Intersects: [1439.63, 1947.14]

#[1283.57, 1439.63]
#[1439.63, 1806.04]
#[1806.04, 1947.14]

#755,489#895,774#1000,2000
#802.826,764.752#896.5,774.5#990.308,633.951#1000,2000
    ...
    if isinstance(t1, Interval) and isinstance(t2, Interval):
        ...
        if T == None:
            if t1.end() < t2.begin():
                ...
                unit, _unit = get_unit_first_mp_inside(mpoint1, mpoint2, t1)
                units += [unit]
                _units += [_unit]
                ...
                unit, _unit = get_unit_second_mp_inside(mpoint1, mpoint2, t2)
                units += [unit]
                _units += [_unit]
            ...
            if T.begin() == a0 and T.end() == a1:
                ii += [T]
 
                unit, _unit = get_unit_ms_inside(mpoint1, mpoint2, T)
                units += [unit]
                _units += [_unit]
                ...
                if t1.end() < t2.end():
                    i2[j2] = Interval(T.end(), t2.end(), True, True)
                    j1 += 1
                else:
                    i1[j1] = Interval(T.end(), t1.end(), True, True)
                    j1 += 1
            elif T.end() == a1:
                if t1.begin() < t2.begin():
                    _interval = Interval(t1.begin(), T.begin(), True, True)
                    #ii += [Interval(t1.begin(), T.begin(), True, True)]
                    ii += [_interval]

                    unit, _unit = get_unit_first_mp_inside(mpoint1, mpoint2, _interval)
                    units += [unit]
                    _units += [_unit]
                else:
                    _interval = Interval(t2.begin(), T.begin(), True, True)
                    #ii += [Interval(t2.begin(), T.begin(), True, True)]
                    ii += [_interval]

                    unit, _unit = get_unit_second_mp_inside(mpoint1, mpoint2, _interval)
                    units += [unit]
                    _units += [_unit]
                ...
                if t1.begin() < t2.begin():
                    ...
                    unit = get_unit_first_mp_inside(mpoint1, mpoint2, t1)
                    units += [unit]
                    """
                    ...
                    unit = get_unit_second_mp_inside(mpoint1, mpoint2, t2)
                    units += [unit]
                    """
        ...
        if t2 < t1.begin():
            ...
            print('TODO: create a unit with a point at an instant.')
            sys.exit()
...
#it = [Interval(1283.57, 1439.63, True, False), Interval(1439.63, 1806.04, True, True), Interval(1806.04, 1947.14, False, True)]
#it = [Interval(1283.57, 1439.63, True, True), Interval(1439.63, 1806.04, True, True), Interval(1806.04, 1947.14, True, True)]
...
#794.6998,569.81745,805.624,592.056,816.5482,614.29455#794.6998,569.81745,792.70868414,569.350420449,790.9445,568.69085#1283.57,1439.63:1283.57,1439.63
#816.5482,614.29455,867.8456,718.7214#816.5482,614.29455,867.8456,718.7214#1439.63,1806.04:1439.63,1806.04
...
mms = create_moving_segment(units, pass_through_control_points, _units)
#sys.exit()

#get_samples_for_out3(msegs1, mms, mpoint1, mpoint2, interval, n_samples, it)
get_samples_for_out4(msegs1, mms, mpoint1, mpoint2, interval, n_samples, it)

#get_samples_for_out2(msegs1, mpoint1, mpoint2, interval, n_samples, it, i2)

operation = Operation.Intersection.value

if print_intersection_information:
    print(get_intersection_information(it, msegs1, mpoint1, operation))

#if print_intersection_information:
#    print(get_intersection_information(i2, msegs1, mpoint2, operation))
...
or operation == STOperation.Intersection.value:
    get_mr_mp_intersections(reg_m_segs, mpoint1, initial_state, final_state, operation, p_linear_traj)
else:
